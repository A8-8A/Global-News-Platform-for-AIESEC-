// Post detail — full article + comments (route "/feed/:id"). Rendered
// inside <Layout>. Article header, hero photo, body, engagement bar, and
// a working comments thread. Like + comment go through React Query
// mutations (optimistic like). Rebuilt from screens-misc.jsx
// <PostDetailScreen>.
...
import {
  HeartIcon, HeartFillIcon, CommentIcon, ShareIcon, BookmarkIcon, ArrowIcon,
} from '../components/ui/Icon';

/* Body paragraphs from post.content, split on blank lines. */
function ArticleBody({ content }) {
  ...
  if (!paras.length) {
    return (
      <p className="font-sans text-ink-soft" style={{ fontSize: 18, lineHeight: 1.7, margin: 0 }}>
        No further detail was filed with this story.
      </p>
    );
  }
  return paras.map((p, i) => (
    <p key={i} className="font-sans text-ink" style={{ fontSize: 18, lineHeight: 1.7, margin: i === paras.length - 1 ? 0 : '0 0 22px' }}>
      {p}
    </p>
  ));
}
...
function Comment({ c, isFirst }) {
  ...
  return (
    <div
      className="grid grid-cols-[40px_1fr] gap-3.5 py-5"
...
export default function PostDetail() {
  ...
  if (isLoading) {
    return (
      <div className="mx-auto max-w-article px-10 py-12">
  ...
  // Detail endpoint returns { post, comments }; tolerate a bare post too.
  ...
  return (
    <div className="bg-paper">
      ...
      <div className="border-b border-line bg-paper-soft">
        <div className="mx-auto max-w-[1080px] px-10 py-3.5 flex items-center justify-between font-mono uppercase text-ink-faint" style={{ fontSize: 11, letterSpacing: '0.14em' }}>
          <span>
            ...
            {post.officeCode ? <><span className="text-ink-soft">{post.officeCode}</span><span className="mx-2.5">›</span></> : null}
      ...
      <article className="mx-auto max-w-article px-10 pt-12 pb-4">
        <div className="flex items-center gap-2.5 mb-4.5" style={{ marginBottom: 18 }}>
          {post.tag && <Pill tone="solid" size="sm">{post.tag}</Pill>}
          {post.officeCode && <OfficeTag code={post.officeCode} mode="chip" />}
          <span className="font-mono text-ink-faint" style={{ fontSize: 11, letterSpacing: '0.1em' }}>
        ...
        <div className="flex items-center gap-3.5 pt-4.5 mt-7" style={{ paddingTop: 18, borderTop: '1px solid var(--line)' }}>
          ...
          <div className="flex flex-col">
            ...
            <span className="font-sans text-ink-faint" style={{ fontSize: 12 }}>{post.authorOffice || ''}</span>
          </div>
      ...
      <div className="mx-auto max-w-body px-10 mt-12">
        <div
          ...
          style={{ padding: '18px 22px', borderRadius: 8, background: 'var(--accent-tint)', border: '1px solid var(--accent-light)' }}
        >
          <button
            type="button"
            ...
            style={{
              ...
              background: liked ? 'var(--accent)' : '#fff',
              color: liked ? '#fff' : 'var(--accent-deep)',
              border: liked ? 'none' : '1px solid var(--accent-light)',
            }}
          >
      ...
      <section className="mx-auto max-w-body px-10 mt-10 pb-20">
        ...
        {/* compose box */}
        <div
          className="mt-4.5 flex gap-3.5 items-start"
          style={{ marginTop: 18, padding: 18, background: '#fff', border: '1px solid var(--line-strong)', borderRadius: 8 }}
        >
          ...
          <div className="flex-1 flex flex-col gap-2.5">
            ...
            <div className="flex items-center justify-between">
              <span className="font-sans text-ink-faint" style={{ fontSize: 11 }}>
                Comments are public and signed with your AIESEC EXPA name.
              </span>
              {isAuthenticated ? (
                ...
                <Btn variant="primary" size="sm" trailing={<ArrowIcon />} onClick={() => navigate('/login')}>
                  Sign in to comment
                </Btn>
              )}
            </div>
        ...
        <div className="mt-6 flex flex-col">
          {comments.length === 0 ? (
            <p className="font-sans text-ink-faint py-8 text-center" style={{ fontSize: 14 }}>
